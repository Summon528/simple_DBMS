...
CMD_t cmd_list[] = {
    {".exit", 5, BUILT_IN_CMD}, {".output", 7, BUILT_IN_CMD},
    {".load", 5, BUILT_IN_CMD}, {".help", 5, BUILT_IN_CMD},
    {"insert", 6, QUERY_CMD},   {"select", 6, QUERY_CMD},
    {"", 0, UNRECOG_CMD},
};
...
int add_Arg(Command_t *cmd, const char *arg) {
    ...
    if (cmd->args == NULL) {
        ...
        memset((void *)new_buf, 0, sizeof(char *) * (cmd->args_cap + 5));
